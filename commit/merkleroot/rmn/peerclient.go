...
type PeerClient interface {
	Send(rmnNodeID rmntypes.NodeID, request []byte) error
...
type peerClient struct {
	...
	rmnPeerIDs mapset.Set[rmntypes.NodeID]
	respChan   chan PeerResponse
}

func NewPeerClient(peer ragep2ptypes.PeerInfo, rmnNodeIDs []rmntypes.NodeID) PeerClient {
	return &peerClient{
		peer:       peer,
...
func (r *peerClient) Send(rmnNodeID rmntypes.NodeID, request []byte) error {
